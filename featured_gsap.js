...
$("#btn_read_more_1").on('click', () => {
    ...
    if(isLessThan_768(width)){
        if(!opensMob[0]){
            gsap.to("#sc_services_item_content_1_bottom", {duration: durationTiming, height: "auto", opacity:"1"});
            opensMob[0] = true;
            $("#btn_read_more_1").text("-")
        } else {
            gsap.to("#sc_services_item_content_1_bottom", {duration: durationTiming, height: "0px", opacity:"0"});
            opensMob[0] = false;
            $("#btn_read_more_1").text("+")
        }
    } else {
      if(curIdx != 1){
          ...
          setTimeout(() => {
              ...
              featuredElem3.style.display = "none"
              if(!open){
                  gsap.to("#featured_content", {duration: durationTiming, height: "auto"});
                  open = true;
              }
              setBtnText()
          }, 500)
...
$("#btn_read_more_2").on('click', () => {
  ...
  if(isLessThan_768(width)){
      if(!opensMob[1]){
          gsap.to("#sc_services_item_content_2_bottom", {duration: durationTiming, height: "auto", opacity:"1"});
          opensMob[1] = true;
          $("#btn_read_more_2").text("-")
      } else {
          gsap.to("#sc_services_item_content_2_bottom", {duration: durationTiming, height: "0px", opacity:"0"});
          opensMob[1] = false;
          $("#btn_read_more_2").text("+")
      }
  } else {
...
$("#btn_read_more_3").on('click', () => {
    ...
    if(isLessThan_768(width)){
      if(!opensMob[2]){
          gsap.to("#sc_services_item_content_3_bottom", {duration: durationTiming, height: "auto", opacity:"1"});
          opensMob[2] = true;
          $("#btn_read_more_3").text("-")
      } else {
          gsap.to("#sc_services_item_content_3_bottom", {duration: durationTiming, height: "0px", opacity:"0"});
          opensMob[2] = false;
          $("#btn_read_more_3").text("+")
      }
  } else {
